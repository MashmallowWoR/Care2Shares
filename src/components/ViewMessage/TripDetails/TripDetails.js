...
class TripDetails extends Component {
	...
	render() {
		const { title, startDate, endDate, personCapacity, listId, reservationData, refundStatus } = this.props;
		const { formatMessage } = this.props.intl;
		const { basePrice, cleaningPrice, weeklyDiscount, monthlyDiscount, userType, currency, cancelData } = this.props;
		// const { paidRefund, paidCurrency} = this.state;

		let checkIn, checkOut, isCancelled;
		checkIn = startDate != null ? moment(startDate).format('ddd, Do MMM') : '';
		checkOut = startDate != null ? moment(endDate).format('ddd, Do MMM') : '';
		isCancelled = cancelData ? true : false;
