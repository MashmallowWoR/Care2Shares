...
class HouseRules extends Component {
  ...
  render() {
    const { hostDisplayName, houseRules, houseRuleDescription } = this.props;
    ...
    return (
      <Panel className={s.houseRulesPanel}>
        ...
        {houseRuleDescription && <div>
          <span className={cx(s.subText, s.lineBreak)}>{houseRuleDescription}</span>
        </div>
        }
      </Panel>
    );
