...
class PaymentForm extends Component {
	...
	render() {
		...
		const { formatMessage } = this.props.intl;
		const { type, originalAmount, email, payoutId, amount, amountCurrency, paymentMethodId, last4Digits } = this.props;
		let typeLabel;
		if (type === 'host') {
					...
					<Row>
						...
						<Col xs={12} sm={9} md={9} lg={9}>
							<CurrencyConverter
								...
								from={amountCurrency}
							/>
							<FormGroup className={cx(s.formGroup, s.spaceTop1)}>
								<Field
									name='refundAmount'
...
const mapState = (state) => ({
	...
	originalAmount: selector(state, 'originalAmount')
});
