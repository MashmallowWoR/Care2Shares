...
class ViewReservation extends React.Component {
    ...
    render() {
        const { type, data, data: { listData, cancellationDetails, refundStatus, transaction } } = this.props;
        let subTotal;
        if (data) {
                        ...
                        }
                        {
                            data && cancellationDetails && (cancellationDetails.refundToGuest == 0 || cancellationDetails.refundToGuest > 0) && <div className={s.profileViewInner}>
                                ...
                                <span className={cx(s.profileViewMainContent)}> 
                                 <CurrencyConverter amount={cancellationDetails.refundToGuest} from={data.currency} />
                                 <br />
                                    {
                                        data && refundStatus && refundStatus.total && <span> <CurrencyConverter amount={refundStatus.total}
                                        from={refundStatus.currency} />  </span>
                                    }
                                </span>
                            </div>
                        }
