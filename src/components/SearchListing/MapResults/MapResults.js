				...
				<div key={key}>
					...
					>
						{!marker.showInfo && (
							...
							>
								<div
									className={cx(s.customMarkerContainer, {
										[s.hoveredMarker]: marker.hovered == true
									})}
								>
									<div className={s.customMarkerPointBorder} />
									<div className={s.customMarkerContent}>
										<CurrencyConverter amount={marker.basePrice} from={marker.currency} />
									</div>
									<div className={s.customMarkerPoint} />
								</div>
							</CustomOverlayView>
						)}
						{marker.showInfo && (
...
class MapResults extends React.Component {
	...
	render() {
		...
		const { searchByMapValue, guests ,personalized} = this.props;
		return (
