  ...
  app.post('/refund', async function (req, res) {
    ...
    paypal.payout.create(create_payout_json, sync_mode, async function (error, payout) {
      if (error) {
        ...
        var batchId = payout && payout.batch_header && payout.batch_header.payout_batch_id;
        var batchStatus = payout && payout.batch_header && payout.batch_header.batch_status;
        var fees = payout && payout.batch_header && payout.batch_header.fees && payout.batch_header.fees.value;
        var transactionId = payout && payout.items && payout.items[0] && payout.items[0].transaction_id;
